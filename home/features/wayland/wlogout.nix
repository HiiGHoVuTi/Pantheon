...
in
{
  ...
  xdg.configFile."wlogout/style.css".text = ''
    * {
    ...
    window {
      background-color: rgba(${config.pantheon.theming.colors.rgba "base00"});
    }

    button {
      color: rgb(${config.pantheon.theming.colors.rgb "base05"});
      background: rgba(${config.pantheon.theming.colors.rgb "base01"}, 0.2);
      border-radius: 8px;
    ...
    button:focus, button:active, button:hover {
      background-color: rgba(${config.pantheon.theming.colors.rgb "base02"}, 0.5);
      outline-style: none;
    }
