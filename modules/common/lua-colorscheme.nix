...
{ lib, config, inputs, ... }: {
  options.pantheon.colorscheme.lua = lib.mkOption {
    type = lib.types.lines;
    description = "Lua file containing the current colorscheme";
  };

  config.pantheon.colorscheme.lua =
    let
      ...
      mkTransparencyTarget = name:
        let
          color = index: "rgba(${
              config.pantheon.theming.colors.rgb "base${index}"
            },${
      ...
      theme = {
        ...
        rounding = {
          enable = config.pantheon.theming.rounding.enable;
          radius = config.pantheon.theming.rounding.radius;
        };
      };
    in
    e.encode theme;
